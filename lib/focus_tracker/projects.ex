defmodule FocusTracker.Projects do
  @moduledoc """
  ...
  def subscribe_projects(%Scope{} = scope) do
    ...
    Phoenix.PubSub.subscribe(FocusTracker.PubSub, "user:#{key}:projects")
  end

  defp broadcast_project(%Scope{} = scope, message) do
    key = scope.user.id

    Phoenix.PubSub.broadcast(FocusTracker.PubSub, "user:#{key}:projects", message)
  end
