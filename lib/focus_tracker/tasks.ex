defmodule FocusTracker.Tasks do
  @moduledoc """
  The Tasks context.
  """
  ...
  def subscribe_tasks(%Scope{} = scope) do
    ...
    Phoenix.PubSub.subscribe(FocusTracker.PubSub, "user:#{key}:tasks")
  end

  defp broadcast_task(%Scope{} = scope, message) do
    key = scope.user.id

    Phoenix.PubSub.broadcast(FocusTracker.PubSub, "user:#{key}:tasks", message)
  end
